...
export default class CreateEventAttendees extends Component {
  ...
  getBookClubsFromUID = () => {
    ...
    firestore()
      ...
      .where('membersUID', 'array-contains', user.uid).get()
      .then(snapshot => {
        if (snapshot.empty) {
        ...
        snapshot.forEach(doc => {
          ...
          this.setState(prevState => ({
            listOfBookClubs: [...prevState.listOfBookClubs, club],
          }))
        });
      })
  ...
  renderBookClubs = ({ item }) => (
    <TouchableWithoutFeedback onPress={() => this.onBookClubListItemPress(item)}>
      <View style={styles.bookClubListItems}>
        <Text style={styles.headline3}>{item.nameOfBookClub}</Text>
          <SeriesOfAvatars 
  ...
  renderMember = ({item}) => (
    item.isHighlighted ? (
      <ListItem
        ...
        onLongPress={() => this.onMemberInListLongPress(item)}
        title={item.displayName}
        leftAvatar={<AvatarForList avatar={item.avatar} avatarsize={24} displayName={item.displayName} />}
        rightElement={<Button
          icon={
            <Icon
              type="feather"
          ...
          title='Remove'
        />}
        containerStyle = {[ styles.attendeeListItemContainer, styles.selected ]}
      />
    ) : (
      <ListItem
        Component={TouchableOpacity}
  ...
  onMemberInListLongPress = member => {
    ...
    const indexOfSelectedMember = this.state.selectedBookClubMembers.findIndex(listItem => member.uid === listItem.uid);
  ...
  render() {
    return (
      <GreyWhiteBackgroundBottomButton
        ...
        />
        {this.state.listOfBookClubs.length > 0 && !this.state.showListOfMembers &&
          <>
